/*
 * saved-prices.js: the prices the user stored: the list, the popups to save,
 * overwrite, rename and delete, and the banner of the price loaded into the form.
 *
 * localStorage is their local cache, not their home: sync.js fills it from
 * Firestore and listens to the events dispatched here. The conversation with
 * the cloud runs both ways without an import both ways: sync.js imports the two
 * functions below, and hears about every change through CustomEvents on the
 * document, so nothing here ever imports sync.js.
 */

import { brl, pct } from './calc.js?v=4';
import { sortPricesByName } from './price-sort.js?v=2';
import { escapeHtml } from './format.js?v=1';
import { readInputs, getLastCalc, loadInputs, setOnReset } from './calculator.js?v=1';
...
/** Binds the list, the four popups and the banner, then paints the list. */
export function attachSavedPrices() {
  ...
  // save popup: with a price already loaded, ask whether to overwrite it or
  // save as new (same as the app); otherwise go straight to asking for the name.
  ...
  $('saveConfirm').addEventListener('click', saveAsNewPrice);
  ...
  $('btnOverwrite').addEventListener('click', overwriteLoadedPrice);
